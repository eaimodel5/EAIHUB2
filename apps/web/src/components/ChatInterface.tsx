...
import { updateProfile, fetchProfile } from '../services/profileService';
...
export const ChatInterface: React.FC = () => {
  ...
  const interventionPollRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => { 
      setSsotVersion(getEAICore().metadata.version); 
      setHasApiKey(checkApiKeyConfigured());
      ...
      const initProfile = async () => {
          try {
              ...
              console.warn("Profile fetch failed, showing setup as fallback");
              setShowProfileSetup(true);
          }
      };
  ...
  useEffect(() => { if (window.innerWidth >= 1024) messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);
  useEffect(() => { analysisRef.current = currentAnalysis; }, [currentAnalysis]);

  // --- INTERVENTION BUS (TEACHER PUSH) ---
  useEffect(() => {
      const userId = getOrCreateUserId();
      const pollKey = `eai_intervention_${userId}`;

      // Clear any stale commands on mount
      localStorage.removeItem(pollKey);

      interventionPollRef.current = setInterval(() => {
          const raw = localStorage.getItem(pollKey);
          if (raw) {
              try {
                  const event = JSON.parse(raw);
                  console.log("[StudentClient] Intervention received:", event);
                  
                  // Clear bus immediately
                  localStorage.removeItem(pollKey);

                  if (event.type === 'COMMAND_INJECTION' && event.payload?.command) {
                      setToastMessage(`Docent activeert: ${event.payload.command}`);
                      setTimeout(() => setToastMessage(null), 3000);
                      // Execute logic
                      handleSendMessage(event.payload.command);
                  }
              } catch (e) {
      ...
      return () => {
          if (interventionPollRef.current) clearInterval(interventionPollRef.current);
      };
  }, []);
  ...
  return (
    <div className={`flex h-[100dvh] w-full ${currentTheme.bg} text-slate-200 overflow-hidden font-sans transition-colors duration-[1500ms] relative`}>
      ...
      {/* Teacher Toast */}
      {toastMessage && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[90] bg-indigo-600 text-white px-6 py-2 rounded-full shadow-2xl animate-in slide-in-from-top-4 flex items-center gap-2">
      ...
      <ProfileSetup 
            ...
            onComplete={(p, g) => { 
                ...
                // Initial kick-off with specific goal
      ...
      <div className={`hidden lg:flex w-64 flex-col ${currentTheme.sidebar} transition-colors duration-[1500ms] z-10 relative border-r ${currentTheme.border}`}>
        <div className="p-6">
             <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setShowTechReport(true)}>
                ...
                <span className="font-semibold text-slate-300 tracking-tight group-hover:text-white transition-colors">Studio 10.0</span>
             </div>
        </div>
        ...
        <div className="p-4 space-y-2">
             ...
             <button 
                onClick={() => navigate('/')}
                className="w-full py-3 rounded-lg text-xs font-semibold tracking-wide flex items-center justify-center px-4 hover:bg-white/5 text-slate-500 hover:text-white transition-all uppercase"
             >
                 Exit Session
             </button>
        </div>
      ...
      <div className={`flex-1 flex flex-col h-full relative z-0 transition-all duration-300 ${isDesktopDashboardOpen ? 'lg:mr-96' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 scrollbar-hide">
            <div className="max-w-3xl mx-auto min-h-full flex flex-col justify-end pb-4">
                ...
                {/* EMPTY STATE - Actionable Starter Buttons */}
                {messages.length === 0 && !isBooting && !showProfileSetup && (
                    <div className="my-auto w-full flex flex-col items-center animate-in fade-in zoom-in duration-700">
                        <div className="w-16 h-16 bg-slate-800 rounded-2xl rotate-45 flex items-center justify-center mb-8 shadow-2xl shadow-black/50">
                            <span className="-rotate-45 text-3xl">💠</span>
                        </div>
                        {learnerProfile.name ? (
                            <h3 className="text-xl font-light text-white mb-2 text-center">
                                Hoi <span className="text-teal-400 font-bold">{learnerProfile.name}</span>, ik heb <span className="text-teal-400 font-bold">{learnerProfile.subject}</span> ({learnerProfile.level}) geladen.
                            </h3>
                        ) : (
                            <h3 className="text-xl font-light text-white mb-2">EAI Core Active</h3>
                        )}
                        <p className="text-sm text-slate-500 mb-8 max-w-xs text-center">
                            De didactische engine is gereed. Wat is je doel voor deze sessie?
                        </p>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-md">
                            <button onClick={() => setShowProfileSetup(true)} className="p-4 bg-teal-900/20 border border-teal-500/30 rounded-xl hover:bg-teal-900/40 transition-all text-left group">
                                <div className="text-teal-400 font-bold text-xs uppercase mb-1 group-hover:text-teal-300">Wijzig Profiel</div>
                                <div className="text-slate-300 text-sm">Pas vak of niveau aan</div>
                            </button>
                            <button onClick={() => handleSendMessage("Ik wil oefenen met Biologie (VWO).")} className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl hover:bg-slate-800 transition-all text-left group">
                                <div className="text-slate-400 font-bold text-xs uppercase mb-1 group-hover:text-white">Snelstart</div>
                                <div className="text-slate-300 text-sm">Biologie Pilot</div>
                            </button>
                        </div>
                    </div>
                )}
