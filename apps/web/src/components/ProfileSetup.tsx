...
export const ProfileSetup: React.FC<ProfileSetupProps> = ({ onComplete, isOpen, currentProfile }) => {
  ...
  const goNext = (e?: React.FormEvent) => {
      ...
      // Validation Logic
      if (step === 3) {
          if (!formData.subject && !customSubject) return; // Block empty
          if (customSubject) {
  ...
  const handlePresetSubjectSelect = (subject: string) => {
      ...
      setCustomSubject(''); // Clear custom if preset selected
      handleStepChange(4);
  };
                    ...
                    {step === 3 && (
                        <div className="w-full max-w-3xl text-center">
                            ...
                            <div className="flex flex-col items-center gap-6 w-full">
                                ...
                                {/* GENERIC INPUT (ALWAYS VISIBLE) */}
                                <div className={`w-full max-w-md ${formData.level !== 'VMBO' ? 'border-t border-white/10 pt-8 mt-4' : ''}`}>
                                    ...
                                    <div className="relative">
                                        ...
                                        <button 
                                            ...
                                            type="button"
                                            className={`absolute right-2 top-2 bottom-2 px-3 rounded-lg transition-all flex items-center justify-center ${customSubject ? 'bg-teal-600 text-white' : 'bg-transparent text-slate-700'}`}
                                        >
                                            →
                                        </button>
                                    </div>
