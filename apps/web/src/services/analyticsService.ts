...
// WRITE INTERVENTION TO STORAGE BUS
export const pushIntervention = async (studentId: string, type: string, payload: any) => {
    // Logic: In a real backend, this would push to a DB/Socket.
    // In this local demo, we write to a specific LocalStorage key that the student client polls.
    
    const busKey = `eai_intervention_${studentId}`;
    const event = {
        type,
        payload,
        timestamp: Date.now()
    ...
    console.log(`[TeacherBus] Pushing to ${busKey}`, event);
    localStorage.setItem(busKey, JSON.stringify(event));
